...
# ==========================================
# 1950년대 풍 3D 프로펠러 비행 슈팅 게임 (다국어 & 매뉴얼 강화판)
# ==========================================
...
# ---------------------------------------------------------
# 1. 다국어 번역 시스템 및 설정 저장
# ---------------------------------------------------------
translations = {
    'en': {
        'start_title': 'Flight Manual',
        'start_msg': 'Ready to fly your 1950s propeller plane?',
        'yes': 'START (YES)',
        'score': 'Score',
        'enemies': 'Enemies',
        'timer': 'Time Left',
        ...
        'controls': 'Controls:',
        'mouse': '- Mouse: Rotate / Left Click: Continuous Fire',
        'space': '- SPACE: Call Enemy / R: Reset',
        'pause_key': '- P: Pause/Resume',
        'diff_key': '- 1~0 keys: Level Select',
        'bg_key': '- B: Change Sky / ESC: Quit',
        'rules': 'Rules:',
        'rule1': '- Destroy the enemy within 60 seconds.',
        'rule2': '- Enemy evasion Level (1-10) can be selected.',
        'diff_select': 'Level Select',
        'close': 'Close',
        'notif_spawn': 'Target Spotted! Engage!'
    },
    'ko': {
        'start_title': '비행 지침서',
        'start_msg': '1950년대 프로펠러 기 비행을 시작하시겠습니까?',
        'yes': '비행 시작 (YES)',
        'score': '점수',
        'enemies': '적기',
        'timer': '남은 시간',
        ...
        'controls': '조작 방법:',
        'mouse': '- 마우스: 기체 회전 / 왼쪽 클릭: 기관총 연사',
        'space': '- SPACE: 적기 호출 / R: 초기화',
        'pause_key': '- P: 일시정지/재개',
        'diff_key': '- 숫자키 1~0: 난이도 설정',
        'bg_key': '- B: 배경 변경 / ESC: 종료',
        'rules': '게임 규칙:',
        'rule1': '- 60초 안에 적기를 격추하면 승리합니다.',
        'rule2': '- 1~10단계까지 적기의 회피 수준 선택 가능.',
        'diff_select': '난이도 선택',
        'close': '닫기',
        'notif_spawn': '적기가 나타났습니다! 추격하세요!'
    }
}
...
# ---------------------------------------------------------
# 2. 전역 상태 및 UI 요소 미리 선언 (참조 오류 방지)
# ---------------------------------------------------------
score = 0
difficulty = 1
game_state = 'MENU' # MENU, WAITING, PLAYING, WIN, LOSE
timer = 60
...
def update_ui_text():
    ...
    enemy_count_text.text = f"{t['enemies']}: {len(active_enemies)}"
    ...
    for i, btn in enumerate(diff_buttons):
        btn.color = color.orange if i + 1 == difficulty else color.black66

    # 패널들 실시간 갱신 (열려 있을 때만)
    if 'help_panel' in globals() and help_panel.enabled:
        create_help_panel()
        help_panel.enabled = True
    if 'start_panel' in globals() and start_panel.enabled:
        create_start_panel()
        start_panel.enabled = True
    ...
    def update(self):
        ...
        if game_state == 'PLAYING':
            ...
            if timer <= 0:
                ...
                for e in enemies:
                    if e and e.enabled:
                        destroy(e.marker); destroy(e.radar_dot); destroy(e)
...
class Enemy(Entity):
    ...
    def update(self):
        ...
        target_pos = player.position + player.forward * 70 + player.right * self.offset.x + player.up * self.offset.y
        self.position = lerp(self.position, target_pos, time.dt * 3)
        self.look_at(self.position + player.forward)
    ...
    def destroy_enemy(self):
        ...
        for _ in range(20):
            p = Entity(model='sphere', color=random.choice([color.orange, color.yellow, color.red, color.gray]), position=self.position, scale=random.uniform(0.3, 1.2))
        ...
        explode.animate_scale(12, duration=0.3); explode.fade_out(duration=0.3); destroy(explode, delay=0.3)
        destroy(self.marker); destroy(self.radar_dot); destroy(self)

# ---------------------------------------------------------
# 5. 게임 제어 함수 (스폰, 초기화, 언어변경, 시작)
# ---------------------------------------------------------
def request_enemy():
    global game_state, timer
    if game_state in ['WAITING', 'WIN', 'LOSE']:
        ...
        enemies.clear()
        game_state = 'PLAYING'; timer = 60
        enemies.append(Enemy(position=player.position + player.forward * 60))
        update_ui_text()

def reset_game():
    global game_state, timer, score
    game_state = 'WAITING'; timer = 60; score = 0
    for e in enemies:
        if e and e.enabled: destroy(e.marker); destroy(e.radar_dot); destroy(e)
...
def create_start_panel():
    ...
    start_panel = WindowPanel(
        title=t['start_title'],
        content=(
            Text(t['start_msg'], origin=(0,0)), Text(''),
            Text(t['controls']), Text(t['mouse']), Text(t['space']), Text(t['rules']), Text(t['rule1']),
            Text(''), Text('Language / 언어:'),
            ButtonGroup(('English', '한국어'), on_selection_changed=lambda i: save_language('en' if i==0 else 'ko')),
...
def change_background():
    ...
    sky.color = sun.color = scene.fog_color = bg['sky']
...
for i in range(1, 11):
    btn = Button(text=str(i), parent=camera.ui, scale=(0.03, 0.03), position=(0.85, 0.35 - (i * 0.04)), color=color.black66, highlight_color=color.lime, on_click=Func(set_difficulty, i))
